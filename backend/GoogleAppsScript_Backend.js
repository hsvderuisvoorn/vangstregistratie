...
function werkGrafiekenBij() {
  ...
  try { bouwJaaroverzicht(); } catch (e) { /* stille fout: geen blokkering bij openen */ }
}

/* ============================================================
 *  JAAROVERZICHT (tabblad "Jaaroverzicht")
 *  Kies in cel B2 een jaar -> tabel + beide grafieken tonen
 *  alleen de vangsten van dat jaar.
 * ============================================================ */

function bouwJaaroverzicht() {
  var ss = SpreadsheetApp.getActiveSpreadsheet();
  var bron = ss.getSheetByName("Vangsten");
  if (!bron) return;
  var data = bron.getDataRange().getValues();
  var k = kolomIndexen(data);
  if (k.datum < 0 || k.soort < 0 || k.aantal < 0) return;

  var tab = ss.getSheetByName("Jaaroverzicht");
  if (!tab) tab = ss.insertSheet("Jaaroverzicht");

  var vorigeKeuze = String(tab.getRange("B2").getValue()).trim();

  var bestaandeCharts = tab.getCharts();
  for (var c = 0; c < bestaandeCharts.length; c++) tab.removeChart(bestaandeCharts[c]);
  tab.clear();

  /* beschikbare jaren uit de data */
  var set = {};
  for (var r = 1; r < data.length; r++) {
    var j = jaarVanRij(data[r], k);
    if (j) set[j] = 1;
  }
  var lijst = Object.keys(set).sort().reverse();
  var keuze = lijst.indexOf(vorigeKeuze) !== -1
    ? vorigeKeuze
    : (lijst[0] || String(new Date().getFullYear()));

  if (lijst.length) {
    tab.getRange("B2").setDataValidation(
      SpreadsheetApp.newDataValidation().requireValueInList(lijst, true).build());
  }

  tab.getRange("A1").setValue("Jaaroverzicht - kies een jaar");
  tab.getRange("A1").setFontSize(13).setFontWeight("bold");
  tab.getRange("A2").setValue("Jaar:");
  tab.getRange("A2").setFontWeight("bold");
  tab.getRange("B2").setValue(keuze).setFontWeight("bold").setBackground("#fff7e0");
  tab.getRange("A2").setNote("Kies een jaar; de tabel en beide grafieken tonen alleen de vangsten van dat jaar.");
  ...
  for (var r2 = 1; r2 < data.length; r2++) {
    if (String(jaarVanRij(data[r2], k)) !== keuze) continue;
    var so = String(data[r2][k.soort] || "").trim();
    var aan = Number(data[r2][k.aantal]);
    var pl = String(data[r2][k.plaats] || data[r2][k.plaatsGps] || "").trim();
    if (so && aan) { perSoort[so] = (perSoort[so] || 0) + aan; soortenSet[so] = 1; }
    if (pl && aan) perPlaats[pl] = (perPlaats[pl] || 0) + aan;
    tabelRijen.push([
      datumTekst(data[r2][k.datum]),
      so,
      aan,
      String(data[r2][k.visser] || ""),
      pl,
      String(data[r2][k.opmerking] || "")
    ]);
  }
...
/* Kies je een nieuw jaar in cel B2, dan wordt dit overzicht vernieuwd */
function onEdit(e) {
  try {
    var sh = e.range.getSheet();
    if (sh && sh.getName() === "Jaaroverzicht" && e.range.getA1Notation() === "B2") {
      bouwJaaroverzicht();
    }
  } catch (err) { /* stil */ }
}

function datumTekst(w) {
...
function kolomIndexen(data) {
  ...
  return {
    ...
    visser: koppen.indexOf("Visser"),
    opmerking: koppen.indexOf("Opmerking")
  };
}
...
function onOpen() {
  ...
  ui.createMenu("Vangsten")
    ...
    .addItem("Jaaroverzicht opnieuw bouwen", "verversJaaroverzicht")
    .addToUi();
...
function verversJaaroverzicht() {
  try {
    bouwJaaroverzicht();
    SpreadsheetApp.getUi().alert("Jaaroverzicht is bijgewerkt.");
  } catch (err) {
    SpreadsheetApp.getUi().alert("Fout: " + err);
  }
}
